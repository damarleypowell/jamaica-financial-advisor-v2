...
const S: Record<string, CSSProperties> = {
  label: { display: 'block', fontSize: 11, fontWeight: 600, color: SUB, marginBottom: 6, letterSpacing: '.06em', textTransform: 'uppercase', fontFamily: BODY },
  input: { width: '100%', height: 48, borderRadius: 10, background: FIELD, border: `1.5px solid ${BORDER}`, fontSize: 14, padding: '0 14px', color: INK, outline: 'none', boxSizing: 'border-box', transition: 'border .15s, box-shadow .15s, background .15s', fontFamily: BODY },
  inputFocus: { border: `1.5px solid rgba(0,200,83,.55)`, boxShadow: '0 0 0 3px rgba(0,200,83,.1)', background: '#F0F8F2' },
  inputWrap: { position: 'relative' as const },
  btnDark: { width: '100%', height: 50, borderRadius: 12, background: FDARK, color: '#fff', fontWeight: 700, fontSize: 15, border: 'none', cursor: 'pointer', fontFamily: BODY, letterSpacing: '.01em', transition: 'transform .15s cubic-bezier(.34,1.56,.64,1), box-shadow .15s, opacity .15s' },
  ...
  h2: { fontSize: 26, fontWeight: 800, color: INK, margin: 0, lineHeight: 1.2, fontFamily: HEAD, letterSpacing: '-0.025em' },
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 22, padding: '8px 0' }}>
      ...
      <button type="button" onClick={onCta}
        ...
        onMouseEnter={e => { (e.currentTarget as HTMLElement).style.transform = 'translateY(-2px)'; (e.currentTarget as HTMLElement).style.boxShadow = '0 8px 28px rgba(11,26,13,.3)'; }}
  ...
  return (
    <div className="hidden md:flex" style={{
      ...
      <div style={{ position: 'relative', zIndex: 1 }}>
        ...
        <h1 style={{ fontFamily: HEAD, fontSize: 28, fontWeight: 800, color: '#F0F6F1', lineHeight: 1.18, margin: '0 0 12px', letterSpacing: '-0.025em' }}>
          Your edge in<br />
  ...
  return (
    <button type="button" onClick={onClick} disabled={disabled}
      ...
      style={{
        ...
        border: `1.5px solid ${hov ? '#C8D6CA' : BORDER}`,
        background: hov ? '#F8FBF8' : WHITE,
  ...
  return (
    <form onSubmit={submit} style={S.stack}>
      ...
      <button type="submit" disabled={isLoading}
        style={{ ...S.btnDark, opacity: isLoading ? 0.65 : 1, cursor: isLoading ? 'not-allowed' : 'pointer', transform: hov && !isLoading ? 'translateY(-2px)' : '', boxShadow: hov && !isLoading ? '0 8px 28px rgba(11,26,13,.28)' : 'none' }}
        onMouseEnter={() => setHov(true)} onMouseLeave={() => setHov(false)}
  ...
  return (
    <form onSubmit={submit} style={S.stack}>
      ...
      <button type="submit" disabled={dis}
        style={{ ...S.btnDark, opacity: dis ? 0.4 : 1, cursor: dis ? 'not-allowed' : 'pointer', transform: hov && !dis ? 'translateY(-2px)' : '', boxShadow: hov && !dis ? '0 8px 28px rgba(11,26,13,.28)' : 'none' }}
        onMouseEnter={() => setHov(true)} onMouseLeave={() => setHov(false)}
  ...
  return (
    <form onSubmit={submit} style={S.stack}>
      ...
      <button type="submit" disabled={loading}
        style={{ ...S.btnDark, opacity: loading ? 0.6 : 1, cursor: loading ? 'not-allowed' : 'pointer', transform: hov && !loading ? 'translateY(-2px)' : '', boxShadow: hov && !loading ? '0 8px 28px rgba(11,26,13,.28)' : 'none' }}
        onMouseEnter={() => setHov(true)} onMouseLeave={() => setHov(false)}
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 100, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16 }}>
      ...
      <div style={{
        ...
        {/* Form pane — white */}
        <div style={{
          flex: 1, padding: '38px 34px', overflowY: 'auto',
