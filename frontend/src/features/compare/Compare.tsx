...
interface CompareResult {
  symbols: string[];
  stocks: Array<{
    symbol: string; name?: string; price?: number; pctChange?: number; dollarChange?: number;
...
export default function Compare() {
  ...
  const { data, isLoading } = useQuery<CompareResult>({
    ...
    queryFn: async () => {
      const res: any = await apiPost('/api/compare', { symbols: validSymbols });
      // Backend returns { comparison: [...], partial } — map to expected shape
      const stocks = (res?.comparison ?? []).map((s: any) => ({
        symbol: s.symbol,
        name: s.name,
        price: s.price,
        pctChange: s.change,
  ...
  const displayStocks = useMemo(() => {
    ...
    return validSymbols.map(sym => stocks.find(s => s.symbol === sym)).filter(Boolean) as any[];
                  ...
                  {METRICS.map((metric, mi) => (
                    <tr key={metric.key} style={{ borderBottom: '1px solid rgba(255,255,255,.025)', background: mi % 2 === 0 ? 'rgba(255,255,255,.01)' : 'transparent' }}>
                      ...
                      {displayStocks.map((s: any) => {
                        ...
                        const formatted = (metric.fmt as Function)(val);
                        const color = metric.color ? (metric.color as Function)(val) : undefined;
                        return (
